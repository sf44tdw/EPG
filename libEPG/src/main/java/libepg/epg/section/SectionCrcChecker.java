 ...
 * @author normal
 */
public final class SectionCrcChecker {
     ...
     * セクションのCRCを検査する。
     *
     * @param sectionData 検査対象セクションのテーブル識別値からCRCまでの全バイト。<br>
     * @return 0 ビット化け無し。<br> それ以外 ビット化け有り。<br>
     */
    public int checkCrc(byte[] sectionData) {
        int c = 0xffffffff;
        for (byte b : sectionData) {
            c = (c << 8) ^ CrcTable[(((c >> 24) & 0xFF) ^ b) & 0xFF];
        }
        return c;
    }
