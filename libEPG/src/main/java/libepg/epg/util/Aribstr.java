...
public class Aribstr {
    ...
    public static final Charset CS = Charset.forName("ISO-8859-1");
    /**
     * バイト列を文字列化するときの文字コード
     */
    ...
    /**
     * EPGから取得したchar()部分のバイト列を文字列に戻す
     *
     * @param pSrcData 文字列化したいバイト列
     * @return 変換された文字列。空配列やnullが渡されたか、文字列変換中に問題が起きた場合は空文字列。
     */
    public static String AribToString(byte[] pSrcData) {

        try {
            byte[] lpszDst;
            if (pSrcData == null || pSrcData.length <= 0) {
                return "";
            }

            lpszDst = new byte[TABLE_ID.MAX_SECTION_LENGTH.BYTE_4093.getMaxSectionBodyLength()];
            int dwSrcPos = 0;
            int dwDstLen = 0;
            int dwSrcData;
            ...
            while (dwSrcPos < pSrcData.length) {
                    ...
                    if ((dwSrcData >= 0x21) && (dwSrcData <= 0x7E)) {
                        ...
                        switch (dwSrcData) {
                            ...
                            case 0x1B:
                                ...
                                break;	// ESC
                            case 0x89:
                                m_emStrSize = STRING_SIZE.STR_MEDIUM;
                                break;	// MSZ
                            case 0x8A:
                                m_emStrSize = STRING_SIZE.STR_NORMAL;
                                break;	// NSZ
            ...
            byte[] temp = new byte[dwDstLen];
            System.arraycopy(lpszDst, 0, temp, 0, temp.length);
            return new String(temp, Aribstr.thisEncoding);
        } catch (UnsupportedEncodingException e) {
            LOG.error("文字列変換に失敗しました。空文字が返ります。 バイト列=" + Hex.encodeHexString(pSrcData), e);
            return "";
        }
    }
    ...
    //
    private static int PutKanjiChar(byte[] lpszDst, int ptr, final int wCode) throws UnsupportedEncodingException {
        ...
        byte[] utf8Bytes = new String(code, "ISO-2022-JP").getBytes(thisEncoding);
    ...
    private static int PutAlphanumericChar(byte[] lpszDst, int ptr, final int wCode) throws UnsupportedEncodingException {
        // 英数字全角文字コード変換
        if (acAlphanumericTable == null) {
            acAlphanumericTable = ("　　　　　　　　　　　　　　　　"
                    ...
                    + "ＰＱＲＳＴＵＶＷＸＹＺ［￥］＾＿"
                    + "　ａｂｃｄｅｆｇｈｉｊｋｌｍｎｏ"
                    + "ｐｑｒｓｔｕｖｗｘｙｚ｛｜｝￣　").getBytes(thisEncoding);
        }
    ...
    private static int PutHiraganaChar(byte[] lpszDst, int ptr, final int wCode) throws UnsupportedEncodingException {
        // ひらがな文字コード変換
        if (acHiraganaTable == null) {
            acHiraganaTable = ("　　　　　　　　　　　　　　　　"
                    ...
                    + "ばぱひびぴふぶぷへべぺほぼぽまみ"
                    + "むめもゃやゅゆょよらりるれろゎわ"
                    + "ゐゑをん　　　ゝゞー。「」、・　").getBytes(thisEncoding);
        }
        /*
	#ifdef _UNICODE
    ...
    private static int PutKatakanaChar(byte[] lpszDst, int ptr, final int wCode) throws UnsupportedEncodingException {
        // カタカナ英数字文字コード変換
        if (acKatakanaTable == null) {
            acKatakanaTable = ("　　　　　　　　　　　　　　　　"
                    ...
                    + "バパヒビピフブプヘベペホボポマミ"
                    + "ムメモャヤュユョヨラリルレロヮワ"
                    + "ヰヱヲンヴヵヶヽヾー。「」、・　").getBytes(thisEncoding);
        }
        /*
	#ifdef _UNICODE
    ...
    private static int PutJisKatakanaChar(byte[] lpszDst, int ptr, final int wCode) throws UnsupportedEncodingException {
        // JISカタカナ文字コード変換
    ...
    private static final int PutSymbolsChar(byte[] lpszDst, int ptr, final int wCode) throws UnsupportedEncodingException {
        ...
        byte[] newB = newS.getBytes(thisEncoding);
    ...
    private static void ProcessEscapeSeq(final int byCode) {
        ...
        switch (m_byEscSeqCount) {
            ...
            case 3:
                ...
                } else if (DesignationDRCS(m_byEscSeqIndex, byCode)) {
                    m_byEscSeqCount = 0;
                    return;
                }
    ...
    private static final boolean DesignationGSET(final int byIndexG, final int byCode) {
        ...
        switch (byCode) {
            case 0x42:
                m_CodeG[byIndexG] = CODE_KANJI;
                return true;	// Kanji
            case 0x4A:
                m_CodeG[byIndexG] = CODE_ALPHANUMERIC;
                return true;	// Alphanumeric
            case 0x30:
                m_CodeG[byIndexG] = CODE_HIRAGANA;
                return true;	// Hiragana
            case 0x31:
                m_CodeG[byIndexG] = CODE_KATAKANA;
                return true;	// Katakana
            case 0x32:
                m_CodeG[byIndexG] = CODE_MOSAIC_A;
                return true;	// Mosaic A
            case 0x33:
                m_CodeG[byIndexG] = CODE_MOSAIC_B;
                return true;	// Mosaic B
            case 0x34:
                m_CodeG[byIndexG] = CODE_MOSAIC_C;
                return true;	// Mosaic C
            case 0x35:
                m_CodeG[byIndexG] = CODE_MOSAIC_D;
                return true;	// Mosaic D
            case 0x36:
                m_CodeG[byIndexG] = CODE_PROP_ALPHANUMERIC;
                return true;	// Proportional Alphanumeric
            case 0x37:
                m_CodeG[byIndexG] = CODE_PROP_HIRAGANA;
                return true;	// Proportional Hiragana
            case 0x38:
                m_CodeG[byIndexG] = CODE_PROP_KATAKANA;
                return true;	// Proportional Katakana
            case 0x49:
                m_CodeG[byIndexG] = CODE_JIS_X0201_KATAKANA;
                return true;	// JIS X 0201 Katakana
            case 0x39:
                m_CodeG[byIndexG] = CODE_JIS_KANJI_PLANE_1;
                return true;	// JIS compatible Kanji Plane 1
            case 0x3A:
                m_CodeG[byIndexG] = CODE_JIS_KANJI_PLANE_2;
                return true;	// JIS compatible Kanji Plane 2
            case 0x3B:
                m_CodeG[byIndexG] = CODE_ADDITIONAL_SYMBOLS;
